<div>
    <div class="flex flex-col items-start mb-6 md:flex-row md:justify-between md:items-center">
        <div class="md:w-3/5">
            <x-modal-title
                title="Slider"
                description="Manage slider photos"
            />
        </div>
    ...
    <div>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3" wire:loading.class.delay="opacity-50">
            @forelse($sliderImages as $index=>$sliderImage)
                <div
                    wire:key='beneficiary-{{ $sliderImage->id }}'
                    class="relative w-full h-56 bg-gray-100 border border-gray-200 rounded"
                    style="background: url('{{ $sliderImage->imageUrl }}'); background-repeat: no-repeat; background-position: center; background-size: cover;"
                >

                    <div class="absolute top-0 right-0 flex items-center p-1 mt-1 mr-1 space-x-2 bg-black rounded bg-opacity-60">
                        <x-button.primary wire:click="openForm({{ $sliderImage }})">
                            <x-heroicon-o-pencil class="w-4 h-4"/>
                        </x-button.primary>
                        <x-button.danger wire:click="confirmDelete({{ $sliderImage }})">
                    ...
                    <div class="grid gap-3 grids-cols-1">
                        <div>
                            <div class="flex items-center justify-center w-full h-64 overflow-hidden bg-gray-100 rounded-lg md:mt-6">
                                @if ($photo)
                                    <img src="{{ $photo->temporaryUrl() }}" alt="Slider photo" class="w-auto h-64">
                                @elseif ($editSlider && $editSlider->imageUrl)
                                    <img src="{{ $editSlider->imageUrl }}" alt="Slider photo" class="w-auto h-64">
                                @else
